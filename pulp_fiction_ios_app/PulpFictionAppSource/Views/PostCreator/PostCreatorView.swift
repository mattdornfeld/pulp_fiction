...
struct PostCreatorReducer: ReducerProtocol {
    private let logger: Logger = .init(label: String(describing: PostCreatorReducer.self))
...
struct PostCreatorView: View {
    ...
    let store: ComposableArchitecture.StoreOf<PostCreatorReducer> = Store(
        initialState: PostCreatorReducer.State(),
        reducer: PostCreatorReducer()
    )
    var body: some View {
